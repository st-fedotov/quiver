...
class FiniteField(Field):
    ...
    def find_matrix_coordinates(self, basis, target):
        ...
        # Do row reduction once on the entire augmented matrix
        ref, p = self.row_reduce(augmented, ncols=n)
        
        # Check if any pivot rows have their first nonzero element in the right part
        for i in range(p):
            nonzero_cols = np.nonzero(ref[i, :])[0]
            if len(nonzero_cols) > 0 and nonzero_cols[0] >= n:
                return None  # Pivot in target columns, not basis columns
        
        # Check if all systems are consistent
        for i in range(p, m):
            if not all(ref[i, n:] == 0):
            ...
            for j in range(n):
                if ref[i, j] == 1:
                    ...
                    if is_first:
                        ...
                        break
            if pivot_col != -1:
                # Calculate this variable's value for ALL systems at once
                solution[pivot_col, :] = ref[i, n:]
                # Adjust for other variables' contributions
                for j in range(pivot_col+1, n):
                    if ref[i, j] != 0:
                        solution[pivot_col, :] = solution[pivot_col, :] - ref[i, j] * solution[j, :]
...
class ComplexNumbers(Field):
    ...
    def left_kernel_basis(self, matrix: np.ndarray) -> np.ndarray:
        ...
        Returns:
            A matrix whose rows form a basis for the left kernel
        """
        return sla.null_space(matrix.T).T
    ...
    def row_reduce(self, matrix: np.ndarray, ncols: Optional[int] = None) -> Tuple[np.ndarray, int]:
        ...
        return self._py_row_reduce(matrix, ncols)

    def _py_row_reduce(self, matrix, ncols):
        """
        Pure Python implementation of row reduction as a fallback.
        This closely follows the algorithm from galois._domains._linalg.row_reduce_jit.
        """
    ...
    def find_matrix_coordinates(self, basis, target, field=None):
        ...
        # Get dimensions
        m, n = basis.shape  # m×n basis matrix
        if len(target.shape) == 1:
            # Single vector case, reshape to column vector
            target = target.reshape(-1, 1)
        m2, k = target.shape  # m×k target matrix (should have m2 == m)
        if m != m2:
            raise ValueError(f"Dimension mismatch: basis has {m} rows but target has {m2} rows")
