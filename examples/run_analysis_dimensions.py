...
def dimension_vector_to_map(dimensions: List[int]) -> Dict[int, int]:
    """Convert a dimension vector list into a vertex-indexed mapping."""
    return {index: value for index, value in enumerate(dimensions)}
...
def run_pipeline(config: Dict[str, Any]) -> None:
    ...
    ambient_map = dimension_vector_to_map(ambient_dim)
    target_map = dimension_vector_to_map(target_dim)
    ...
    if quiver_type == "An":
        ...
        coverage_dir = process_coverage(
            Q, F, coverage_name, ambient_map, target_map, output_dir,
            runtime["workers"],
            runtime["r_max"],
        ...
        try:
            ...
            _, jobs = make_rad_jobs_for_Dn(
                n_vertices,
                Q,
                coverage=ambient_map,
                target_dim=target_map,
            )
