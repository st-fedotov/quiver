"""
Coverage analysis pipeline for quiver representations.

Provides end-to-end analysis workflow for a single coverage vector,
including RAD computations, rank posets, visualizations, and conjecture checking.
"""
...
def process_coverage(Q, F, coverage_name, ambient_dim, target_dim, output_dir, workers, r_max, hilbert_workers, gc_heap_size, check_hilbert=True):
    """
    ...
    # Step 1: Enumerate all interval bags for this coverage
    print(f"[{coverage_name}] Step 1: Enumerating interval bags...")
    n = len(Q.get_vertices())
    _, jobs = make_rad_jobs_for_An(n, Q, coverage=ambient_dim, target_dim=target_dim)
    print(f"[{coverage_name}] Enumerated {len(jobs)} interval multisets.")

    # Step 2: Write batch RAD jobs
    print(f"[{coverage_name}] Step 2: Writing batch RAD jobs...")
    batch_dir = write_batch_rad_from_interval_bags_explicit(
        jobs,
        field=F,
    ...
    print(f"[{coverage_name}] Step 5: Building rank poset...")

    # Create poset_input.json
    dir_edges = build_dir_edges(jobs[0][0])
    jobs_json = []
    for _, bag, td in jobs:
        jobs_json.append({
            "bag": [[int(a), int(b)] for (a, b) in bag],
        })

    poset_input_path = coverage_dir / "poset_input.json"
    with open(poset_input_path, "w") as f:
        json.dump({"dir_edges": dir_edges, "jobs": jobs_json}, f)

    poset_out_dir = coverage_dir / "rank_poset"
    poset_out_dir.mkdir(exist_ok=True)

    # Run rank_poset_parallel.py
    result = subprocess.run(
        [
            "python3", "rank_poset_parallel.py",
            "--input", str(poset_input_path),
            "--out", str(poset_out_dir),
            "--workers", str(workers)
        ],
        capture_output=True,
        text=True
    ...
    if result.returncode != 0:
        print(f"[{coverage_name}] WARNING: rank_poset_parallel.py failed!")
        print(f"STDOUT: {result.stdout}")
        print(f"STDERR: {result.stderr}")
    else:
        print(f"[{coverage_name}] Rank poset built: {result.stdout.strip()}")

    # Determine generic_dim from parsed.csv (needed for visualization and conjectures)
    ...
    print(f"[{coverage_name}] Step 5b: Generating visualization with magic_number={generic_dim}...")
    try:
        ...
        print(f"[{coverage_name}] Visualization saved: {viz_output_base}.svg")
    except Exception as e:
        print(f"[{coverage_name}] WARNING: Visualization failed: {e}")

    # Step 6: Check conjectures
    print(f"[{coverage_name}] Step 6: Checking conjectures...")
    ...
    try:
        check_conjectures(
            ...
            orientation="degenerates_to",
            generic_dim=generic_dim,
            jobs=jobs,
        )
        print(f"[{coverage_name}] Conjectures checked. Results in {reports_dir}")
    except Exception as e:
        print(f"[{coverage_name}] WARNING: Conjecture checking failed: {e}")
